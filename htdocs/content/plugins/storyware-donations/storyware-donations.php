...
/**
 * Plugin Name: Storyware Donations
 * Plugin URI: http://www.storyware.us
 * Description: Plugin for donations. 
 * Version: 1.0.0
 * Author: Storyware
 * Author URI: http://www.storyware.us
 ...
 */
defined('STORYWARE_DONATIONS') ? STORYWARE_DONATIONS : define('STORYWARE_DONATIONS', 'storyware-donations');
 ...
 */
$vars = [
    'slug' => 'storyware-donations',
    'name' => 'Storyware Donations',
    'namespace' => 'us.storyware.donations',
    'config' => 'us_storyware_donations',
];
...
add_action('admin_notices', function () use ($vars) {
    if (!class_exists('\Themosis\Foundation\Application')) {
        printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', STORYWARE_DONATIONS));
    }
    ...
    if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
        printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('Your application does not handle the following plugin: ', STORYWARE_DONATIONS), $vars['name']);
    }
});
...
container('action')->add('plugins_loaded', function () use ($vars) {
	 ...
	 */
	load_plugin_textdomain(STORYWARE_DONATIONS, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
